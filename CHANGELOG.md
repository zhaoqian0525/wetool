...
### Changed
- 工具详情页操作栏重排：全屏/安装固定靠右（方便手机单手操作），分享按钮改为纯图标（箭头式分享图标）
- 使用记录、换封面、删除移到工具下方；顶部保留举报入口
- 作者 / 改编自 / 日期 / 浏览量合并到同一行
...
### Changed
- 手机端工具详情页头部压缩：作者与「改编自」合并同一行，日期/浏览量放到次行；「能力说明」移到工具下方，顶部更清爽，全屏按钮保持在上方醒目位置
...
### Added
- 发布成功后自动弹出分享卡片，包含工具封面、标题、一句话介绍、二维码和「由 XXX 制作」，可一键「保存分享图」（生成 PNG 下载）

### Changed
- 工具详情页操作栏优化：收藏按钮改为纯图标（去掉文字省空间），二维码按钮换用正确的二维码图标
...
### Added
- 工具详情页操作栏新增「📱 二维码」按钮：一键生成当前工具的二维码，手机扫码即可打开使用，支持复制链接
...
### Changed
- 电脑优先模式下，右侧桌面预览窗口加宽（最大放宽到 1280px），显示面积更大
- 桌面端预览窗口顶部新增「⛶ 全屏」按钮，可直接全屏预览
...
### Changed
- 创作页桌面端把「代码」和「预览」合并为一个面板（顶部切换），显示面积更大，默认显示预览；手机端三 tab 行为不变
- 工具详情页「公开/未列出/私密」可见性下拉从 hover 改为点击展开（兼容触屏）
...
### Changed
- 创作页切换「电脑优先」时，右侧预览同步切换为桌面浏览器窗口（1280×720，按容器等比缩放），不再是手机框；切回手机优先则恢复手机预览
...
### Removed
- 删除内置工具「酒店比价小助手」及其 mock 评论、封面文件
...
### Removed
- 删除内置工具「API 权限测试台」（体验不佳）与「科学计算器 Pro」，同步清理 sitemap、内置工具 UUID 映射与 SEO 文案中的引用
...
### Added
- 教程页新增「平台能做什么 / 不能做什么」能力边界说明，明确列出可用的沙盒能力与不支持的能力（任意联网、服务器、工具内登录/支付、麦克风/摄像头、通知/定位、读剪贴板等）
...
### Added
- 内置「API 权限测试台」工具（第 19 个内置工具）：逐项验证云记忆 KV、localStorage 持久化、剪贴板、文件导出、分享、用户信息、语音朗读、白名单联网、工具内 AI、使用记录、Web Worker、图片导入等沙盒 API 是否正常

### Changed
- 路线图优先级调整：创作者激励/作者信誉暂缓，原生 App 后置；近期优先打磨移动端 PWA 体验 + 招牌工具 + 轻量推广 + 域名备案评估
...
### Added
- 审核处理结果通知：管理员下架/恢复工具时通知作者；举报被处理/驳回时通知举报人（通知中心「系统」类型）
