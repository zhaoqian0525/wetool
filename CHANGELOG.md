...
### Fixed
- 缩短工具 localStorage 墓碑同步间隔（800ms → 300ms），并在页面隐藏/离开时立即强制同步，降低刚记录就刷新导致最后一条数据丢失的窗口
...
### Fixed
- “使用记录 → 清空全部记录”现在会同时清空该工具保存的记忆数据（云端状态与本地墓碑），并重载工具为空状态，避免清完记录后工具数据仍残留
...
### Fixed
- 修复慢网络下工具记忆“晚到丢失”：状态接口超过 10s 时页面先按空状态挂载工具，云端记忆到达后不再只注入不重读，而是允许 iframe 用正确快照重建
...
### Fixed
- 恢复工具 iframe 的 `allow-modals`：此前 AI 生成的工具用 `alert` / `confirm` 做校验提示，沙盒静默屏蔽弹窗，导致“开始使用”等按钮点击无反应
- AI 提示词同步要求：校验反馈优先用页面内文字/toast，避免过度依赖原生弹窗
...
### Fixed
- 修复拍照识别无反应：`__wewoo.pickImage` 现在自动压缩图片（最长边 1600、JPEG 0.9），避免 iPhone 原图 base64 超过视觉模型输入上限被截断，导致识别失败
...
### Changed
- 工具详情页首屏提速：工具本体先加载渲染，评论/评分延后加载，避免慢接口阻塞工具 iframe
- 全局增加 Supabase 预连接（preconnect / dns-prefetch），降低冷启动建连耗时
...
### Fixed
- 修复登录态尚未确认时过早把工具 iframe 挂载为“游客空状态”，导致云端记忆晚于工具初始化注入、工具不重读 localStorage，跨设备打开时减肥记录器/2048 等进度看起来丢失
- 现在会等待登录态确认后再加载云端状态并挂载 iframe，保证首帧就带上正确的 `_ls` 快照
...
### Fixed
- 修复 iOS「添加到主屏幕」偶发保存成官网首页：把站点 manifest 与工具 manifest 的注入合并为根组件单一链接，消除客户端导航时的链接竞争，无需刷新即使用正确的工具 start_url
...
### Fixed
- 修复工具状态读取失败（网络抖动/接口错误）时被误判为“无历史记录”，随后仅写入 `_opened` 标记，覆盖云端已有 `_ls` / `_draft` / `__wewoo.save` 记忆数据，导致游戏进度等数据丢失
- 状态接口读取失败现在返回 500，前端不再覆盖写入；`useToolStorage` 同样只对成功响应做云端回填
...
### Changed
- AI 上下文改为“预算制”动态保留历史：系统提示词 + 当前代码优先，剩余预算从最新往旧保留用户需求，短消息多保留、长消息自动少保留，最新一条需求始终保留
- 回收部分上限：输出 token 48000 → 32000，上下文护栏 200000 → 150000，单条需求 20000 → 12000
- 历史条数硬上限 32 → 40（实际保留数量由上下文预算决定），单条历史最多 8000 字
...
### Changed
- AI 生成上限再放松：代码上限 3.2 万 → 10 万字符，输出 token 1.6 万 → 4.8 万，上下文护栏 4.8 万 → 20 万字符
- 单条需求上限 8000 → 20000 字，历史消息条数 24 → 32，压缩历史保留更多用户上下文
- AI 系统提示词补充代码长度说明，复杂工具可写得更完整
...
### Fixed
- 手机端操作栏按钮间距微调（gap 与内边距缩小），安装按钮回到第一行
...
### Changed
- 举报按钮移到工具下方，与使用记录/换封面/删除同一行
- 电脑端新增「下载」按钮（下载工具 HTML），安装按钮移到手机第一行
- 修复电脑端预览框与按钮行贴得太近的间距问题
...
### Changed
- 电脑端操作栏合并为单行（安装上移、全屏按钮加宽）；手机端仍保持两行（全屏/安装靠右）
...
### Changed
- 二维码按钮去掉「二维码」文字，改为纯图标
- 用户名 / 日期 / 浏览量 / 改编自合并到同一行
...
### Changed
- 工具详情页操作栏改为两行：第一行点赞/收藏/分享/二维码/改编，第二行全屏/安装靠右
...
### Fixed
- 工具详情页操作栏按钮恢复单行显示（修复两行错排），全屏/安装靠右，窄屏可横向滑动
...
### Changed
- 工具详情页头部 meta 恢复两行排版（第一行作者/改编自，第二行日期/浏览量），全屏/安装保持靠右
