...
3.5 零风险平台 API（回调风格，均可用；复制/导出/分享/用户信息/朗读等场景优先用这些，不要用 navigator.clipboard 或直接下载）：一键复制 __wewoo.copyText('文本', function(err){ ... })；文件导出 __wewoo.download('文件名.txt', '内容', 'text/plain', function(err){ ... })（可导出 CSV/报告/图片 base64 等）；分享 __wewoo.share({ title: '标题', text: '文案', url: '链接' }, function(err){ ... })；读取当前用户昵称头像 __wewoo.getUser(function(err, res){ res.name / res.avatar }，只读昵称头像、拿不到邮箱/ID）；语音朗读 __wewoo.speak('文字', { lang: 'zh-CN', rate: 1, pitch: 1 }, function(err){ ... })；音效可用 WebAudio（AudioContext/OscillatorNode）合成。所有调用前用 if (window.__wewoo && __wewoo.xxx) 特性检测，失败时降级提示。大数据/图片处理可用 new Worker(URL.createObjectURL(new Blob(['...'], {type:'application/javascript'}))) 防卡 UI（沙盒支持 blob Worker）。图片导入用 <input type="file" accept="image/*"> + FileReader 转 base64，再用 __wewoo.ai.chat({ prompt: '...', image: 'data:image/...' }) 做识别（不要用 getUserMedia 摄像头）。
