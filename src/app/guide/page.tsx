...
const steps = [
  ...
  },
  {
    ...
    desc: "在创作页点击编辑器下方「直接用 AI 生成」区域，输入你想做的工具（比如：做一个纪念日记录器，能添加重要日子、显示倒数天数），点「开始生成」，AI 会边写边显示代码；完成后点「填入编辑器」即可。",
    tip: "内置 AI 已自动遵循微坞的要求：手机适配 375px、localStorage 记忆、沙盒安全合规；也可以继续用外部 AI（ChatGPT、Kimi 等），把下面的提示词发给它们",
    code: true,
  },
  {
    num: 3,
  ...
  },
  {
    ...
    desc: "用内置 AI 生成的话，直接点「填入编辑器」就能自动填入；用外部 AI 的话，把代码复制后粘贴到左边黑色的编辑器里（Ctrl+V 或长按粘贴）。右边会立刻显示效果，",
